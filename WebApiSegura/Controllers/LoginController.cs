    ...
    public class LoginController : ApiController
    {
        ...
        public IHttpActionResult Authenticate(LoginRequest login)
        {
            ...
            if (!string.IsNullOrEmpty(usuarioValidado.USU_IDENTIFICACION))
            {
                ...
                usuarioValidado.CadenaToken = token; // agregada
        ...
        private Usuario ValidarUsuario(LoginRequest loginRequest)
        {
            ...
            {
                SqlCommand sqlCommand = new SqlCommand("SELECT USU_CODIGO, USU_IDENTIFICACION, USU_NOMBRE, USU_PASSWORD, USU_EMAIL, USU_ESTADO, USU_FEC_NAC, USU_TELEFONO FROM  USUARIO WHERE USU_IDENTIFICACION = @USU_IDENTIFICACION", sqlConnection);
                sqlCommand.Parameters.AddWithValue("@USU_IDENTIFICACION", loginRequest.Username); //prevenir ataque SQL
        ...
        public IHttpActionResult Register(Usuario usuario)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["RESERVAS"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@" INSERT INTO USUARIO (USU_IDENTIFICACION, USU_NOMBRE, USU_PASSWORD, USU_EMAIL, USU_FEC_NAC, USU_ESTADO, USU_TELEFONO) VALUES (@USU_IDENTIFICACION, @USU_NOMBRE, @USU_PASSWORD, @USU_EMAIL, @USU_FEC_NAC, @USU_ESTADO, @USU_TELEFONO)", sqlConnection);
